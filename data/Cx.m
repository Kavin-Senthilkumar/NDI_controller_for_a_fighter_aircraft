...
function Cx_t = Cx_calculation(Alpha, Beta, Del_h, del_lef, del_sb, c, q, V)
    ...
    function Cx = Cx_lef(Alpha,Beta)
        [alpha, beta, Cx] = read_data('data\aero_coeffs\Cx\Cx,lef(alpha,beta).txt');
    ...
    function Cx = Cx_alpha_beta_del_h_0(Alpha, Beta)
        [alpha, beta, Cx] = read_data('data\aero_coeffs\Cx\Cx(alpha,beta,del_h=0).txt');
        F = scatteredInterpolant(alpha, beta, Cx, "linear");
        Cx = F(Alpha, Beta);
    end
    ...
    function Cx = Cx_alpha_beta_del_h(Alpha,Beta,Del_h)
        data1 = readmatrix('data\aero_coeffs\Cx\Cx(alpha,beta,del_h=-25).txt','Delimiter',',');
        data2 = readmatrix('data\aero_coeffs\Cx\Cx(alpha,beta,del_h=-10).txt','Delimiter',',');
        data3 = readmatrix('data\aero_coeffs\Cx\Cx(alpha,beta,del_h=0).txt','Delimiter',',');
        data4 = readmatrix('data\aero_coeffs\Cx\Cx(alpha,beta,del_h=10).txt','Delimiter',',');
        data5 = readmatrix('data\aero_coeffs\Cx\Cx(alpha,beta,del_h=25).txt','Delimiter',',');
    ...
    function Cx = del_Cx_sb(Alpha)
        [alpha, Cx] = read_data1("data\aero_coeffs\Cx\delCx,sb(alpha).txt");
    ...
    function Cx = Cx_q(Alpha)
        [alpha, Cx] = read_data1("data\aero_coeffs\Cx\Cxq(alpha).txt");
        Cx = interp1(alpha, Cx, Alpha, 'linear');
    end
    %fprintf('%.4f', Cx_q(20));

    function Cx = del_Cx_q_lef(Alpha)
        [alpha, Cx] = read_data1("data\aero_coeffs\Cx\delCxq,lef (alpha).txt");
        Cx = interp1(alpha, Cx, Alpha, 'linear');
    end
